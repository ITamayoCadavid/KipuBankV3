...
## 6) Recomendaciones para validar invariantes

* Hacer tests simples para comprobar que la **suma de balances de los usuarios coincida con los totales del contrato** (`totalETHDeposits` y `totalUSDCDeposits`).
  
* Verificar que los **totales nunca superen `bankCap`**.
  
* Comprobar que **ningún usuario pueda tener balance negativo** después de retiros.
  
* Revisar que en los swaps de tokens, la cantidad recibida de USDC **sea al menos la esperada**.
  
* Usar eventos para **registrar depósitos, retiros y swaps** y facilitar la revisión.
  
* Ejecutar tests de depósitos y retiros con distintos escenarios, incluyendo valores extremos.

---

## 7) Próximos pasos necesarios para alcanzar la madurez del protocolo y completar la preparación para una auditoría

* Crear tests para **depósitos y retiros** de ETH y USDC.
  
* Probar swaps de tokens y verificar que **no haya pérdida de fondos**.
  
* Añadir un **mecanismo de pausa** (`Pausable`) para emergencias.
  
* Revisar que **todos los balances internos coincidan con los fondos reales** del contrato.
  
* Documentar cómo ejecutar los tests y qué eventos se emiten.
  
* Probar el contrato en **testnet**, usando un router simulado y, si es posible, Uniswap V2 de testnet.
...
KipuBankV3 **todavía no está listo para mainnet ni auditoría externa**.
Se deben cubrir los tests básicos, añadir mecanismos de emergencia y validar los swaps.
Con estas mejoras, el contrato podría considerarse más seguro y maduro para un proceso de auditoría.
